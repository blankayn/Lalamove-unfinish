  ...
  const handleRegister = async (e) => {
    ...
    try {
      const res = await fetch(`${API}/register.php`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: regEmail, phone: regPhone, password: regPassword, country }),
      });
      const data = await res.json();
      setRegMsg(data.message);
      if (data.success) setTimeout(() => setIsRegister(false), 1500);
    } catch {
      setRegMsg('Cannot connect to server. Make sure XAMPP is running.');
    }
  };
          ...
          {!isRegister ? (
            ...
            <form onSubmit={handleLogin} className="space-y-3">
              <select
                ...
                className="h-11 w-full rounded border border-slate-300 bg-white px-3 text-sm text-slate-700 outline-none focus:border-[#f36f21]"
              >
              ...
              <button type="submit" disabled={loading}
                className="h-11 w-full rounded bg-[#f36f21] text-sm font-bold text-white transition hover:brightness-105 disabled:opacity-60">
              ...
              <button type="button" className="flex h-11 w-full items-center justify-center gap-3 rounded border border-slate-300 bg-white text-sm font-semibold text-[#1877f2] hover:bg-slate-50">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="#1877f2"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                Continue with Facebook
              </button>

              <button type="button" className="flex h-11 w-full items-center justify-center gap-3 rounded border border-slate-300 bg-white text-sm font-semibold text-slate-700 hover:bg-slate-50">
              ...
              <p className="text-center text-sm text-slate-500">
                New to Lalamove?{' '}
                <span onClick={() => { setIsRegister(true); setError(''); }} className="cursor-pointer font-semibold text-[#f36f21] hover:underline">
            ...
            <form onSubmit={handleRegister} className="space-y-3">
              ...
              <input type="password" placeholder="Password (min 6 characters)" value={regPassword} onChange={e => setRegPassword(e.target.value)} required
                className="h-11 w-full rounded border border-slate-300 px-3 text-sm outline-none placeholder:text-slate-400 focus:border-[#f36f21]" />

              {regMsg && (
                <p className={`text-xs ${regMsg.includes('success') ? 'text-green-600' : 'text-red-500'}`}>{regMsg}</p>
              )}
              ...
              <p className="text-center text-sm text-slate-500">
                ...
                <span onClick={() => { setIsRegister(false); setRegMsg(''); }} className="cursor-pointer font-semibold text-[#f36f21] hover:underline">
                  Log In
                </span>
              </p>
            </form>
